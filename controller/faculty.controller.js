...
const fs = require("fs");
const { createTokenFaculty } = require("../utils/token");
...
const getLogin = (req, res) => {
	res.render("Faculty/signin");
};
const postLogin = (req, res) => {
	try {
		const { facultynumber, password } = req.body;
		const findUser = "SELECT * from faculty WHERE facultynumber = ?";

		db.query(findUser, [facultynumber], async (err, result) => {
			if (err) {
				...
				if (result.length > 0) {
					...
					if (match_password) {
						const generateToken = createTokenFaculty(
							result[0].facultynumber
						);
						...
						req.flash(
							"error_msg",
							"Incorrect faculty number or password"
						);
						res.redirect("/faculty/signin");
					}
				} else {
...
const postRegister = async (req, res) => {
	...
	const { facultynumber, name, phonenumber, email, password, gender } =
		req.body;
	let errors = [];
	...
	//Query statement
	const student_count = (await queryParam(student_exist, [facultynumber]))[0]
		.count;
	...
	if (student_count > 0) {
		errors.push({ msg: "Faculty number is already registered" });
	}
...
const getDashboard = async (req, res) => {
	const student = (
		await queryParam(
			"SELECT facultynumber, name, email, phonenumber, join_date,gender from faculty WHERE facultynumber = ?",
			[res.locals.id]
		)
	)[0];
...
const getAllBooks = async (req, res) => {
	...
	res.render("Faculty/all_books", { title: "All Books", booksData });
};
	...
	db.query(
		...
		(duplicateErr, duplicateResult) => {
			if (duplicateErr) {
				...
				return res.redirect("/faculty/all_books");
			}
	  ...
	  `;
		...
		`;
				...
				db.query(msgQuery, [userId, message_content], (err) => {
					...
					req.flash(
						"success_msg",
						`Book title ${book_name} issued successfully`
					);
					res.redirect("/faculty/all_books");
				});
			});
		}
	);
...
const getBorrowedBooks = (req, res) => {
	...
	db.query(query, [userId], (err, results) => {
		...
		res.render("Faculty/borrowed_books", {
...
const getIssuedBooks = (req, res) => {
	...
	db.query(query, [res.locals.id], (err, results) => {
		...
		res.render("Faculty/issued_books", {
			title: "Issued Books",
			issuedBooks: results,
		});
	});
...
const getAttendance = async (req, res) => {
	...
	res.render("Faculty/attendance", { title: "Attendance", attendances });
};
const getMessages = async (req, res) => {
	...
	res.render("Faculty/messages", { title: "Messages", messages });
};
const getRecommendation = (req, res) => {
	res.render("Faculty/recommendations", { title: "Recommendations" });
};
const postRecommendation = (req, res) => {
	const { book_name, description, author } = req.body;
	const facultynumber = res.locals.id;
	const data = {
		book_name,
		description,
		author,
		user_id: facultynumber,
	};
	...
	db.query("INSERT INTO recommendations SET ?", data, (err, result) => {
		if (err) {
			...
			res.redirect("/faculty/recommendations");
		} else {
